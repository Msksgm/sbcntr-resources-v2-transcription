...
.PHONY: aws.create-db-subnet-group-cfn
aws.create-db-subnet-group-cfn: aws.define-db-subnet-group-variables ## AuroraインスタンスのDBサブネットグループを作成
	@aws cloudformation create-stack --stack-name ${DB_SUBNET_GROUP_STACK_NAME} \
		--template-body file://handson/cloudformations/db_subnet_group.yml \
		--parameters \
			ParameterKey=SubnetDbA,ParameterValue=$(SUBNET_DB_A) \
			ParameterKey=SubnetDbC,ParameterValue=$(SUBNET_DB_C)
	@echo "${DB_SUBNET_GROUP_STACK_NAME}: 作成中です（約1分かかる）"
	@time aws cloudformation wait stack-create-complete --stack-name ${DB_SUBNET_GROUP_STACK_NAME}

.PHONY: aws.delete-db-subnet-group-cfn
aws.delete-db-subnet-group-cfn: ## AuroraインスタンスのDBサブネットグループを削除
	@echo 'Before status'
	@make aws.status
	@aws cloudformation delete-stack --stack-name $(DB_SUBNET_GROUP_STACK_NAME)
	@echo '削除中です'
	@time aws cloudformation wait stack-delete-complete --stack-name $(DB_SUBNET_GROUP_STACK_NAME)
	@echo 'After status'
	@make aws.status

.PHONY: aws.show-defined-db-subnet-group-variables
aws.show-defined-db-subnet-group-variables: aws.define-db-subnet-group-variables ## DBサブネットグループ作成に必要な環境変数を表示
	@echo "SUBNET_DB_A:          $(SUBNET_DB_A)"
	@echo "SUBNET_DB_C:          $(SUBNET_DB_C)"
...
aws.create-all-cfns: ## cfnを作成する
	...
	@make aws.create-db-subnet-group-cfn

.PHONY: aws.delete-all-cfns
aws.delete-all-cfns: ## cfnを削除する
	@make aws.delete-db-subnet-group-cfn
...
aws.define-db-subnet-group-variables:
	$(eval SUBNET_DB_A         := $(shell aws ec2 describe-subnets --filters "Name=tag:Name,Values=sbcntr-private-db-a" --query "Subnets[0].SubnetId" --output text))
	$(eval SUBNET_DB_C         := $(shell aws ec2 describe-subnets --filters "Name=tag:Name,Values=sbcntr-private-db-c" --query "Subnets[0].SubnetId" --output text))
